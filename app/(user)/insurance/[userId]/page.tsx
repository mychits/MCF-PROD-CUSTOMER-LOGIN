...
const Insurance = ({ params }: { params: Promise<{ userId: string }> }) => {
  ...
  const handleEnquiry = async (optionType: string) => {
    const confirm = window.confirm(`Are you sure you want to enquire about ${optionType} insurance?`);
    if (!confirm) return;
    ...
    try {
      ...
      if (response.data.message === "updated enquiry") {
        toast.success(`Success! Your interest in ${optionType} has been recorded.`);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-blue-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 sm:h-20 flex items-center justify-between">
          ...
          <div className="text-center">
             <h1 className="text-slate-900 font-black tracking-tight text-lg uppercase leading-none">Insurance</h1>
             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest hidden sm:block mt-1">Protective Coverage</p>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10">
          <div className={`text-center sm:text-left transform transition-all duration-700 ${isMounted ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
            <h2 className="text-3xl sm:text-5xl font-black text-slate-900 mb-2">Insurance</h2>
            <p className="text-slate-500 font-medium italic">Secure what you love</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8">
          {insuranceOptions.map((option, index) => (
            <div 
              ...
              className={`flex flex-col gap-3 transform transition-all duration-700 ${isMounted ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
            >
              {/* Card Container */}
              <div 
        ...
        {/* --- TRUST FOOTER --- */}
        <div className={`mt-20 p-8 bg-white rounded-[3rem] border border-slate-100 flex flex-col md:flex-row items-center gap-8 shadow-sm transition-all duration-1000 delay-500 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
           ...
           <div className="flex-1">
              <h4 className="text-lg font-black text-slate-900 uppercase tracking-tight mb-2">Need a custom plan?</h4>
              <p className="text-slate-500 text-sm leading-relaxed">
                Our advisors are ready to help you find the best coverage for your specific needs. Click any category above or contact your relationship manager for assistance.
              </p>
           </div>
           <button 
