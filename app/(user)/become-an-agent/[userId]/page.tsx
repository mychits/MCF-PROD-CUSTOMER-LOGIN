...
const BecomeAnAgent = ({ params }: { params: Promise<{ userId: string }> }) => {
  ...
  const validateForm = () => {
    const { fullName, email, phoneNumber, address, idProofType, idProofNumber, bankAccountNumber, ifscCode } = formData;
    if (!fullName || !email || !phoneNumber || !address || !idProofType || !idProofNumber || !bankAccountNumber || !ifscCode) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${url}/become-agent/agents/become`, submissionData);
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        setFormData({
          ...
          idProofType: "", idProofNumber: "", bankAccountNumber: "",
          ifscCode: "", experience: ""
        });
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-blue-900/10 overflow-hidden border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="bg-gradient-to-r from-[#053B90] to-blue-700 p-8 sm:p-12 text-center text-white">
            ...
            <h2 className="text-2xl sm:text-4xl font-black tracking-tight mb-3">Agent Application Form</h2>
            <p className="text-blue-100 font-medium opacity-90 max-w-md mx-auto">
              Join our network and start your journey towards professional growth.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div className="md:col-span-2">
                <h3 className="text-xs font-black text-blue-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                  <span className="w-8 h-px bg-blue-600"></span> Personal Information
                </h3>
              </div>
              ...
              <div className="md:col-span-2 relative">
                <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-2 block">Full Address</label>
                <div className="relative">
                  <IoMapOutline className="absolute left-4 top-4 text-slate-400" size={20} />
                  <textarea 
              ...
              <div className="md:col-span-2 mt-4">
                <h3 className="text-xs font-black text-blue-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                  <span className="w-8 h-px bg-blue-600"></span> Identity & Banking
                </h3>
              </div>
              ...
              <InputField 
                ...
                onChange={(val:any) => handleInputChange('bankAccountNumber', val)} 
              ...
              <div className="md:col-span-2 relative mt-4">
                <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-2 block">Relevant Experience</label>
                <div className="relative">
                  <IoBriefcaseOutline className="absolute left-4 top-4 text-slate-400" size={20} />
                  <textarea 
                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-4 pl-12 pr-4 outline-none focus:border-blue-600 focus:bg-white transition-all text-slate-700 font-medium min-h-[100px]"
